...
class SiteSettings(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    site_name = db.Column(db.String(150), default='Dawrzawn')
    ...
    footer_text = db.Column(db.String(500), default='© 2025 Dawrzawn. All rights reserved.')
...
def admin_settings():
    settings = get_settings()
    if request.method == 'POST':
        settings.site_name = request.form.get('site_name', 'Dawrzawn').strip()
        ...
        if 'maps_api_key' in request.form:
...
with app.app_context():
    ...
    if not User.query.filter_by(role='admin').first():
        admin = User(username='admin', email='admin@dawrzawn.com', role='admin')
